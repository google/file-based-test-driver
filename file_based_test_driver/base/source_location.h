...
#ifndef THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_SOURCE_LOCATION_H_
#define THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_SOURCE_LOCATION_H_
...
// `file_based_test_driver_base::SourceLocation`. The caller can then invoke the
// function, passing `FILE_BASED_TEST_DRIVER_LOC` as the argument.
//
...
#define FILE_BASED_TEST_DRIVER_INTERNAL_HAS_KEYWORD(x) !(__is_identifier(x))
#else
#define FILE_BASED_TEST_DRIVER_INTERNAL_HAS_KEYWORD(x) 0
#endif

#if !defined(FILE_BASED_TEST_DRIVER_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT)
#if FILE_BASED_TEST_DRIVER_INTERNAL_HAS_KEYWORD(__builtin_LINE) && \
    FILE_BASED_TEST_DRIVER_INTERNAL_HAS_KEYWORD(__builtin_FILE)
#define FILE_BASED_TEST_DRIVER_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT 1
#else
#define FILE_BASED_TEST_DRIVER_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT 0
#endif
#endif

#undef FILE_BASED_TEST_DRIVER_INTERNAL_HAS_KEYWORD

namespace file_based_test_driver_base {

// Class representing a specific location in the source code of a program.
// `file_based_test_driver_base::SourceLocation` is copyable.
class SourceLocation {
 ...
 public:
  ...
  // by the `FILE_BASED_TEST_DRIVER_LOC` macro, hence the name.
  static constexpr SourceLocation DoNotInvokeDirectly(std::uint_least32_t line,
                                                      const char* file_name) {
    return SourceLocation(line, file_name);
  }

#if FILE_BASED_TEST_DRIVER_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT
 ...
 private:
  ...
  // `FILE_BASED_TEST_DRIVER_LOC` macro below.
  //
  // `file_name` must outlive all copies of the
  // `file_based_test_driver_base::SourceLocation` object, so in practice it
...
// If a function takes an `file_based_test_driver_base::SourceLocation`
// parameter, pass this as the argument.
#define FILE_BASED_TEST_DRIVER_LOC                                             \
  ::file_based_test_driver_base::SourceLocation::DoNotInvokeDirectly(__LINE__, \
                                                                     __FILE__)

#endif  // THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_SOURCE_LOCATION_H_
