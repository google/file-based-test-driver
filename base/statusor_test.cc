...
#include "base/status_matchers_oss.h"
...
TEST(StatusOr, TestMoveOnlyVector) {
  // Check that StatusOr<MoveOnly> works in vector.
...
TEST(StatusOr, TestCopyCtorStatusOk) {
  ...
  FILE_BASED_TEST_DRIVER_EXPECT_OK(copy.status());
...
TEST(StatusOr, TestAssignmentStatusOk) {
  // Copy assignmment
  {
    ...
    FILE_BASED_TEST_DRIVER_EXPECT_OK(target.status());
    EXPECT_EQ(p, target.ValueOrDie());

    ASSERT_TRUE(source.ok());
    FILE_BASED_TEST_DRIVER_EXPECT_OK(source.status());
...
TEST(StatusOr, SelfAssignment) {
  // Copy-assignment, status OK
  {
    ...
    ASSERT_TRUE(so.ok());
    FILE_BASED_TEST_DRIVER_EXPECT_OK(so.status());
