...
#ifndef THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_MAP_UTIL_H_
#define THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_MAP_UTIL_H_
...
const V& ValueInitializedDefault() {
  static const file_based_test_driver_base::NoDestructor<V>
...
const MapUtilMappedT<M>& FindWithDefault(const M& m,
                                         const MapUtilKeyT<M>& key) {
  auto it = m.find(key);
  if (it != m.end()) return file_based_test_driver_base::subtle::GetMapped(*it);
...
bool FindCopy(const M& m, const Key& key, Value* value) {
  ...
  if (value) *value = file_based_test_driver_base::subtle::GetMapped(*it);
  return true;
}
...
bool ContainsKeyValuePair(const M& m, const Key& key, const Value& value) {
  ...
  for (auto it = range.first; it != range.second; ++it) {
    if (file_based_test_driver_base::subtle::GetMapped(*it) == value) {
      return true;
    }
  }
  return false;
...
MapUtilMappedT<M> EraseKeyReturnValuePtr(M* m, const MapUtilKeyT<M>& key) {
  ...
  if (it == m->end()) return nullptr;
  MapUtilMappedT<M> v =
      std::move(file_based_test_driver_base::subtle::GetMapped(*it));
  m->erase(it);
  return v;
}
...
#endif  // THIRD_PARTY_FILE_BASED_TEST_DRIVER_BASE_MAP_UTIL_H_
